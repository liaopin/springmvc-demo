...
public class ZkTest {
	...
	public static void main(String[] args) throws Exception {
        ...
        String gateways = client1.create().creatingParentsIfNeeded()//若创建节点的父节点不存在会先创建父节点再创建子节点
                .withMode(CreateMode.PERSISTENT)//withMode节点类型，
                .forPath("/baymax/gateways",null);
        System.out.println(gateways);
        String systems = client1.create().creatingParentsIfNeeded()//若创建节点的父节点不存在会先创建父节点再创建子节点
                .withMode(CreateMode.PERSISTENT)//withMode节点类型，
                .forPath("/baymax/systems",null);
